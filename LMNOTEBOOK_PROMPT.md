...
- source code, config, script, command, log, code snippet이 있으면 식별자와 동작을 보존한다.
...
문서에 source code, config, script, command, log, code snippet이 포함되어 있으면 반드시 작성한다.
코드가 없으면 이 섹션에는 "문서에서 명시된 코드/스니펫 없음"만 쓰고 하위 섹션은 생략한다.
...
원문에 있는 중요한 코드는 가능한 한 정확히 보존한다. source code 파일 자체가 업로드된 경우 작은 파일은 전체를 보존하고, 긴 파일은 구조 요약과 핵심 함수/설정/명령을 발췌한 뒤 생략한 범위를 설명한다.
...
5. 코드가 있으면 파일 경로, 함수명, 클래스명, config key, command, API endpoint를 entities 또는 aliases에도 반영하라.
